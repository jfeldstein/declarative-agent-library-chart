...
CREATE TABLE IF NOT EXISTS hosted_agents.run_operational_event (
  id BIGSERIAL PRIMARY KEY,
  kind TEXT NOT NULL,
  run_id TEXT NOT NULL,
  thread_id TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS ix_run_ops_run
  ON hosted_agents.run_operational_event (run_id);

CREATE INDEX IF NOT EXISTS ix_run_ops_created
  ON hosted_agents.run_operational_event (created_at DESC);
