...
def _graph_key(ctx: TriggerContext) -> tuple[Any, ...]:
    obs = _obs(ctx)
    use_cp = obs.checkpoints_enabled and not ctx.ephemeral
    return (
        use_cp,
        obs.checkpoint_backend,
        ctx.ephemeral,
    )
...
def compiled_trigger_graph(ctx: TriggerContext) -> Any:
    ...
    obs = _obs(ctx)
    use_checkpointing = obs.checkpoints_enabled and not ctx.ephemeral
    checkpointer = build_checkpointer(obs) if use_checkpointing else None
...
def get_thread_state(thread_id: str, *, ctx_template: TriggerContext | None = None) -> Any:
    ...
    obs = _obs(ctx)
    if not obs.checkpoints_enabled:
...
def get_thread_state_history(thread_id: str, *, ctx_template: TriggerContext | None = None) -> list[Any]:
    ctx = ctx_template or trigger_context_for_admin_reads()
    if not _obs(ctx).checkpoints_enabled:
        msg = "checkpointing is disabled"
        raise RuntimeError(msg)
    graph = compiled_trigger_graph(ctx)
