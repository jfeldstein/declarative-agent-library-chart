...
- **`observability.*`**: cluster scrape hints—`prometheus.io/*` annotations, optional **`ServiceMonitor`**, **`structuredLogs.json`** → `HOSTED_AGENT_LOG_FORMAT=json`, Postgres pool caps for observability tables when configured, etc.
- **`observability.plugins.*`**: Phase 1 **scaffold** (`prometheus`, `langfuse`, `wandb`, `grafana`, `logShipping`; all default **false**). Treat as **no-op** for behavior until release notes document wiring—see [Runtime architecture](#runtime-architecture-lifecycle-events-and-plugins-phase-1).
