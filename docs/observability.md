...
**Prerequisites:** Docker, kind, kubectl, helm, curl, Python 3. Optional: `CLEANUP_KIND=1` to delete the cluster when the script exits; `SKIP_KIND_CREATE=1` to reuse an existing cluster name (`KIND_CLUSTER_NAME`, default `cfha-o11y-it`). Helm installs use `HELM_WAIT_TIMEOUT` (default **20m** for the example chart **`helm --wait`**) and `ROLLOUT_TIMEOUT` (default **600s**). For **`prometheus-community/prometheus`**, the integration script skips **`helm --wait`** (gRPC deadlines on GitHub Actions), **`kubectl patch`es** the server Deployment **`progressDeadlineSeconds`** to **`PROM_PROGRESS_DEADLINE_SEC`** (default **3600**; the chart default **600** is too tight for kind on CI), then runs **`kubectl rollout status`** with **`PROM_ROLLOUT_TIMEOUT`** (default **45m**). Values set **`imagePullPolicy: Never`** on the Prometheus server and config-reloader because images are pre-loaded into kind. The integration script **pre-pulls** the pinned Prometheus chart images for **`PROM_CHART_VERSION` 29.2.0** and **loads them into the kind node** via **`docker cp` + `ctr -n k8s.io images import`** (same as the app image) so the cluster avoids slow quay pulls during `helm install --wait` (see **`runtime/tests/scripts/integration_kind_o11y_prometheus.sh`**). If you bump **`PROM_CHART_VERSION`**, update the preload image list in that script.