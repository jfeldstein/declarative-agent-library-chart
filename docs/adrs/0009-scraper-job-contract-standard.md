...
6. **Non-goals**  
   This contract **SHALL NOT** define the **trigger** HTTP API, **`TriggerBody`**, or inbound **`*-trigger`** bridges (ADR 0010). Enabling the agent trigger deployment **SHALL NOT** by itself deploy RAG; **`declarative-agent.ragDeployed`** is true only when at least one scraper job is enabled under **`scrapers.jira`** or **`scrapers.slack`**, matching the comment on **`scrapers.ragService`** in **`values.yaml`**.
