...
2. **Single assignment, many readers** — Canonical env alias precedence for identity exists in **one module** at construction time only (for example `runtime_identity.py`). Prometheus metrics, optional `wandb_session` tagging, and other consumers use the context object; they **do not** re-resolve the same identity from env for those fields when context supplies values (**context is truth**).
