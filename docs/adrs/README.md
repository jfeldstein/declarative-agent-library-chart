...
- **0016 — [Run identity on TriggerContext and plugin-interpreted lifecycle events](0016-run-identity-on-context-and-plugin-events.md)** — Resolve agent/model identity once at trigger entry; core emits facts on the event bus; plugins map to vendor formats; correlation carries identity for async paths.
- **0017 — [Gate optional capabilities at the call site](0017-gate-at-call-site-for-optional-capabilities.md)** — `if cfg.enabled` before `register_*`; registrations act or raise; no silent no-op when enabled; strict builders after the gate.
