...
**Helm `Chart.yaml` type vs OpenSpec 1.2** — Investigated **`type: library`** for **`helm/chart/`** per **`cfha-helm-library`** / **`config-first-hosted-agents`** task **1.2**. Helm [library charts](https://helm.sh/docs/topics/library_charts/) **do not render** dependent templates that emit Kubernetes manifests, so flipping **`type`** broke **`helm template`** on **`examples/*`** (empty manifest) and **helm-unittest**. **`Chart.yaml`** now documents why the chart stays **`type: application`** while remaining **consumed only via parent charts** under **`examples/`**; OpenSpec tasks **1.2** updated with this blocker. README layout table clarifies “dependency-packaged,” not Helm **`library`** type.
