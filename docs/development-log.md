...
**OpenSpec `postgres-agent-persistence` (rebased on `main`)** — Pinned **`langgraph-checkpoint-postgres`** + **`psycopg[binary]`** as optional **`[postgres]`**. **`HOSTED_AGENT_POSTGRES_URL`** + **`HOSTED_AGENT_POSTGRES_POOL_MAX`** for checkpoints and **`HOSTED_AGENT_OBSERVABILITY_STORE=postgres`**; Helm **`observability.postgresUrl`** / **`postgresUrlSecret`** (with optional nested **`postgres.url`** aliases). Migrations Job, **`postgres_repos`**, **`--extra postgres`** image. DRY with **`postgres_env.postgres_url()`** and PGlite on **`main`**.
