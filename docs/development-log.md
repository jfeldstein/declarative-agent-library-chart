...
**Unified Postgres env + checkpointing example** — **`HOSTED_AGENT_POSTGRES_URL`** is the canonical DSN; legacy checkpoint/observability URL env vars are read only as fallbacks (**`hosted_agents.observability.postgres_env`**). Helm **`observability.postgresUrl`** maps to the unified var (deprecated **`observability.checkpoints.postgresUrl`** still coalesced). New example **`examples/checkpointing/`** enables **`observability.checkpoints`** with **memory** while the library default stays off.

**PGlite + unified Postgres URL** — Optional **`[pglite]`** extra (**`py-pglite[psycopg]`**); **`HOSTED_AGENT_USE_PGLITE=1`** starts TCP-mode PGlite and sets **`HOSTED_AGENT_POSTGRES_URL`** when unset. Shared DSN resolution lives in **`hosted_agents.observability.postgres_env`** (canonical **`HOSTED_AGENT_POSTGRES_URL`**, legacy checkpoint/observability URLs as fallbacks). Runbook: **`docs/runbook-checkpointing-wandb.md`**.
