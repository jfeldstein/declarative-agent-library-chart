...
**Worktree → `main` (flatten Helm)** — `/apply-worktree`: **`main`** had a stalled cherry-pick (**`31e32c9`**, unmerged paths); **`git cherry-pick --abort`** cleared it, then **`38360d0`** was cherry-picked as **`6bcb0b6`** (`refactor(helm): flatten hosted_agents path; move Dockerfile to helm/`). Verified **`scripts/check_spec_traceability.py`**, **`uv run ruff check hosted_agents tests`**, **`uv run pytest tests/`** (101 passed, coverage gate met).
...
**Slack + Jira scrapers (apply)** — **`hosted_agents.scrapers.slack_job`** (`slack_sdk` / `WebClient`, **`SLACK_SCRAPER_SEARCHES_JSON`**) and **`hosted_agents.scrapers.jira_job`** (httpx Jira Cloud **`/rest/api/3/search`** + comments, watermark under **`JIRA_WATERMARK_DIR`**). Helm **`scraper-cronjobs.yaml`** routes by **`integration`** (and **`name: reference`**); chart **`scrapers.jira`** defaults + schema; **`examples/with-scrapers/values-helmtest-*.yaml`** and **`helm/tests/with_scrapers_test.yaml`** assert **`slack_job`** / **`jira_job`** dispatch. **`slack_sdk`** / **`slack-bolt`** in **`helm/src`** deps; coverage omit for the two CronJob modules (still covered by targeted tests + templates) so the **85%** gate stays green.
