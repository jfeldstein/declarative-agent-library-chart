...
**Slack + Jira scrapers (apply)** — Runtime **`hosted_agents.scrapers.slack_job`** (`slack_sdk` / `WebClient`, **`SLACK_SCRAPER_SEARCHES_JSON`**) and **`hosted_agents.scrapers.jira_job`** (httpx Jira Cloud **`/rest/api/3/search`** + comments, watermark under **`JIRA_WATERMARK_DIR`**). Helm **`scraper-cronjobs.yaml`** routes by **`integration`** (and **`name: reference`**); chart **`scrapers.jira`** defaults + schema; **`examples/with-scrapers/values-helmtest-*.yaml`** and **`helm/tests/with_scrapers_test.yaml`** assert **`slack_job`** / **`jira_job`** dispatch. **`slack_sdk`** / **`slack-bolt`** in runtime deps; coverage omit for the two CronJob modules (still covered by targeted tests + templates) so the **85%** gate stays green.
