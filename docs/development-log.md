...
**Scheduled kind + Prometheus integration** — **`runtime/tests/scripts/integration_kind_o11y_prometheus.sh`**: (1) default Helm release **`o11y-kind-it` → `o11y-it`** (CronJob name **≤52**). (2) App + preload images: **`docker save`** to a host tar, **`docker cp`** into the kind control-plane container, then **`ctr -n k8s.io images import`** on a **path** (stdin import still hit deadlines on GitHub Actions). (3) **`Dockerfile`** **`WITH_WANDB`** build-arg (default **1**); integration builds **`--build-arg WITH_WANDB=0`** for a smaller image and faster kind load. (4) **Pre-pull** pinned **quay.io** images for **`prometheus-community/prometheus` 29.2.0** — server **`v2.55.1`** (keeps chart args predictable on kind CI), config-reloader **`v0.90.1`** (**`docker cp` + `ctr import`**). **`prometheus-kind-o11y-values.yaml`**: **`global.scrape_timeout: 2s`** with **`scrape_interval: 3s`** (chart default timeout is **\>3s**, which makes Prometheus **exit 2** on config load). **Prometheus install:** **`helm upgrade --install`** **without** **`--wait`**, **`kubectl patch`** **`progressDeadlineSeconds`**, **`kubectl rollout status`** (**`PROM_ROLLOUT_TIMEOUT`** default **45m**); on rollout failure, **`kubectl logs -c prometheus-server`**. **`prometheus-kind-o11y-values.yaml`** sets **`imagePullPolicy: Never`** on server + config-reloader (images pre-loaded into kind). **`HELM_WAIT_TIMEOUT`** default **20m**; scheduled workflow job timeout **60** minutes. **`docs/observability.md`**: preload + chart bump checklist.
