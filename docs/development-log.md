...
**Python project layout** — The **`uv`** / pytest project previously at **`runtime/`** now lives under **`helm/src/`** (same `src/hosted_agents/`, `tests/`, `pyproject.toml`, and lockfile). CI, root **`Dockerfile`**, **`.dockerignore`**, **`scripts/check_spec_traceability.py`** (pytest evidence under **`helm/src/tests/`**), **`docs/spec-test-traceability.md`**, and contributor docs reference the new path.
