...
- **Python only:** `uv sync --all-groups --project helm/src` then `cd helm/src` and `uv run ruff format --check agent tests`, `uv run ruff check agent tests`, `uv run vulture agent`, `uv run complexipy`, `uv run pytest tests/ -v --tb=short`
