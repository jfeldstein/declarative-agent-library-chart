...
### Requirement: [DALC-REQ-SLACK-TRIGGER-001] App mention events start a hosted agent trigger run
...
### Requirement: [DALC-REQ-SLACK-TRIGGER-002] Trigger path does not ingest Slack mention traffic into managed RAG
...
### Requirement: [DALC-REQ-SLACK-TRIGGER-003] HTTP Events requests are verified when that topology is used
...
### Requirement: [DALC-REQ-SLACK-TRIGGER-004] Trigger configuration keys are distinct from Slack scraper configuration
...
### Requirement: [DALC-REQ-SLACK-TRIGGER-005] Secrets are never written to logs or metrics labels on the trigger path
