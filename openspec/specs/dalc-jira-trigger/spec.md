...
### Requirement: [DALC-REQ-JIRA-TRIGGER-001] Jira webhook events start a hosted agent trigger run
...
### Requirement: [DALC-REQ-JIRA-TRIGGER-002] Trigger path does not ingest Jira webhook traffic into managed RAG
...
### Requirement: [DALC-REQ-JIRA-TRIGGER-003] Webhook requests are verified when that topology is used
...
### Requirement: [DALC-REQ-JIRA-TRIGGER-004] Trigger configuration keys are distinct from Jira scraper configuration
...
### Requirement: [DALC-REQ-JIRA-TRIGGER-005] Secrets are never written to logs or metrics labels on the trigger path
