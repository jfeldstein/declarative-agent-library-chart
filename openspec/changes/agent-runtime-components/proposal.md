...
- **Python coverage (ADR 0002)**: first-party runtime packages under `helm/src/hosted_agents/`, including **`hosted_agents/observability/`**, are measured by **`pytest-cov`** with **no** broad `omit` that hides whole subtrees; landing observability or checkpoint helpers requires accompanying tests like any other runtime code.
