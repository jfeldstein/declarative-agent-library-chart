...
### Requirement: [CFHA-REQ-JIRA-SCRAPER-001] Chart exposes structured `scrapers.jira` values

The Helm chart SHALL publish a **`scrapers.jira`** object in **`values.yaml`** and **`values.schema.json`** for operator-tunable Jira Cloud scraping (including at minimum **`siteUrl`**, **authentication via Secret references**, and a **`projects`** list with per-project **`key`**), without introducing a top-level **`rag`** property.

#### Scenario: Schema documents scrapers.jira

- **WHEN** an operator inspects **`values.schema.json`**
- **THEN** **`properties.scrapers.properties`** SHALL include a documented **`jira`** object suitable for validating **`siteUrl`**, secret-backed credentials, and **`projects[].key`**

### Requirement: [CFHA-REQ-JIRA-SCRAPER-002] Jira scraper job routes to `jira_job` by integration

When a scraper CronJob container sets **`SCRAPER_INTEGRATION`** to **`jira`**, the chart SHALL invoke the **`hosted_agents.scrapers.jira_job`** module (or the documented successor entrypoint) instead of **`stub_job`**, while preserving existing routing for the **`reference`** job and the default stub fallback for unknown integrations.

#### Scenario: Enabled Jira integration uses jira entrypoint

- **WHEN** `helm template` renders an enabled scraper job whose **`SCRAPER_INTEGRATION`** (or equivalent chart field) is **`jira`**
- **THEN** the scraper container command SHALL target **`hosted_agents.scrapers.jira_job`** and SHALL NOT target **`hosted_agents.scrapers.stub_job`**

### Requirement: [CFHA-REQ-JIRA-SCRAPER-003] Incremental Jira sync uses JQL on `updated` with a persisted watermark

The Jira scraper SHALL discover candidate issues using Jira Cloud **enhanced JQL search** (`POST /rest/api/3/search/jql` or the documented successor) with a **project-scoped** query that includes **`updated >= <watermark>`** (including a configurable **overlap** window), SHALL paginate until **caps** or end of result set, and SHALL advance a **durable per-scope watermark** only after a successful RAG embed for the processed batch (or SHALL document and implement an equivalent atomicity strategy that prevents permanent data loss on partial failure).
...
### Requirement: [CFHA-REQ-JIRA-SCRAPER-004] Issue payload includes status, assignee, links, and paginated comments
...
### Requirement: [CFHA-REQ-JIRA-SCRAPER-005] Secrets and metrics stay bounded
