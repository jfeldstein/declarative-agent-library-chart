...
- Rename **`grafana/dalc-agent-overview.json`** → **`grafana/dalc-overview.json`**; replace **`cfha`** product tags/UIDs in that dashboard with **`dalc`** where they denote the product (not OpenSpec requirement IDs).
...
- **Out of scope for this change**: Renaming promoted OpenSpec **requirement ID prefixes** (`[DALC-REQ-…]`, capability folder names under `openspec/specs/`) — that is a separate traceability-wide migration.
...
- **Tests**: Python tests, helm-unittest YAML, integration scripts; **`docs/spec-test-traceability.md`** and test docstrings that cite dashboard paths under **[DALC-REQ-O11Y-LOGS-003]**.
