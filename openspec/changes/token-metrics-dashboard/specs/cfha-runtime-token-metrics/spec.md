...
### Requirement: [CFHA-REQ-TOKEN-MET-001] LLM output token counter
...
### Requirement: [CFHA-REQ-TOKEN-MET-002] LLM input token counter

The agent process SHALL increment **`agent_runtime_llm_input_tokens_total`** for **input** (prompt) tokens reported by the provider for each completed generation, with the **same bounded label set** as **[CFHA-REQ-TOKEN-MET-001]**.
...
### Requirement: [CFHA-REQ-TOKEN-MET-003] Time-to-first-token histogram
...
### Requirement: [CFHA-REQ-TOKEN-MET-004] Trigger payload size histograms
...
### Requirement: [CFHA-REQ-TOKEN-MET-005] Estimated cost counter
...
### Requirement: [CFHA-REQ-TOKEN-MET-006] Metric help text states estimate semantics

Each new metric in **[CFHA-REQ-TOKEN-MET-001]** through **[CFHA-REQ-TOKEN-MET-005]** SHALL include **Prometheus HELP** text stating whether values are **provider-reported**, **runtime-estimated**, or **byte-size approximations**, and **`agent_runtime_llm_estimated_cost_usd_total`** HELP text SHALL explicitly include the word **estimate**.
