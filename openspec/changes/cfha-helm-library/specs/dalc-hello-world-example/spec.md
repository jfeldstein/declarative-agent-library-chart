...
### Requirement: Hello-world example depends on the Helm library chart

The repository SHALL include `examples/hello-world/Chart.yaml` of **`type: application`** that declares the shared **library** chart as a Helm dependency with `repository: file://../../helm/chart` (or equivalent path after the rename) and `values.yaml` containing at minimum:
...
#### Scenario: Example declares file dependency

- **WHEN** a maintainer opens `examples/hello-world/Chart.yaml`
- **THEN** it SHALL reference the shared chart under `helm/chart` such that `helm dependency update` resolves it for local installs
...
4. The response body SHALL include content consistent with the configured `system-prompt` behavior (for example a greeting containing **Hello** and the wave emoji **:wave:** or **👋** as rendered in the actual response format).
