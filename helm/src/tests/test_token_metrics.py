...
def test_trigger_payload_histograms_record_response_size(
    ...
    after = _metrics_text(client)
    assert "dalc_http_trigger_response_bytes" in after
    assert _histogram_sum(after, "dalc_http_trigger_response_bytes") >= (
        _histogram_sum(before, "dalc_http_trigger_response_bytes")
        + out_len
        - 1e-9
    )
...
def test_trigger_payload_histograms_record_request_size(
    ...
    assert "dalc_http_trigger_request_bytes" in after
    assert after.count("dalc_http_trigger_request_bytes") >= before.count(
