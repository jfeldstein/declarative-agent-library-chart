"""Tests for Jira REST tools (`hosted_agents.tools_impl.jira`)."""
...
from hosted_agents.tools_impl.jira.config import load_settings
from hosted_agents.tools_impl.jira.rest import normalize_jira_error
from hosted_agents.tools_impl.jira.router import TOOL_IDS
...
def test_jira_tools_python_sources_avoid_embed_route():
    ...
    root = Path(__file__).resolve().parents[1] / "hosted_agents" / "tools_impl" / "jira"
...
def test_real_search_uses_httpx(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "hosted_agents.tools_impl.jira.handlers.build_client", lambda _s: FakeClient()
    )
...
def test_token_not_in_tool_error_payload(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "hosted_agents.tools_impl.jira.handlers.build_client", lambda _s: BoomClient()
    )
...
def test_router_rejects_unknown_tool(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _enable_jira_tools(monkeypatch)
    from hosted_agents.tools_impl.jira.router import invoke as jinvoke
...
def test_real_get_issue(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "hosted_agents.tools_impl.jira.handlers.build_client",
