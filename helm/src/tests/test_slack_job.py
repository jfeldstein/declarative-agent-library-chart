...
def test_redact_token_like_strips_slack_token_patterns() -> None:
    """[DALC-REQ-SLACK-SCRAPER-005] Redact token-like substrings from stderr/logging paths."""
...
def test_normalize_slack_channel_requires_conversation_id() -> None:
    with pytest.raises(SystemExit) as excinfo:
        slack_job._normalize_slack_job(
            {"source": "slack_channel", "conversationId": ""}
        )
    assert excinfo.value.code == 1
...
def test_run_slack_search_posts_embed(
    ...
    """[DALC-REQ-SLACK-SCRAPER-003] RTS search path posts normalized items to RAG ``/v1/embed``."""
    cfg = {
        "source": "slack_search",
        "query": "hello",
