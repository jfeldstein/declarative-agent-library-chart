...
class LangfusePluginSettings:
    """Langfuse OTLP/SDK plugin config (maps from ``observability.plugins.langfuse`` Helm keys)."""
...
def plugins_config_from_env() -> ObservabilityPluginsConfig:
    """Parse plugin toggles + Langfuse scalar fields used by Helm ``env`` wiring."""

    lf = LangfusePluginSettings(
        enabled=_truthy("HOSTED_AGENT_LANGFUSE_ENABLED"),
    ...
    return ObservabilityPluginsConfig(langfuse=lf)
