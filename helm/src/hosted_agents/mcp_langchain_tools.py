...
def _tool_slack_chat_update(safe_name: str, ctx: TriggerContext) -> Any:
    @tool(safe_name, description="Update a Slack message posted by the bot (chat.update).")
...
def _tool_jira_search_issues(safe_name: str, ctx: TriggerContext) -> Any:
    @tool(safe_name, description="Search Jira issues with JQL (bounded by deployment caps).")
...
def _tool_jira_transition_issue(safe_name: str, ctx: TriggerContext) -> Any:
    @tool(safe_name, description="Transition a Jira issue; use transition_id or transition_name.")
