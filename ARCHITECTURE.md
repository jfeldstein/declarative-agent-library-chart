...
- **Image supply:** Point `image.repository` / `image.tag` at a built image that contains the **`agent`** package (built with [`helm/src/Dockerfile`](helm/src/Dockerfile) from the repository root; see README).
